...
fit <- fit_res %>%
  ...
  group_by(Event.ID) %>%
  # create ANOVA table
  do(tidy(lm(value ~ variable, data = .))) %>%
  # replace NA with -
  na.fill("-")
# write table
knitr::kable(
...
data_summary_2$company <- "All (112)*"
...
```{r plot-clustering-regression, fig.cap="Regression of clustering with abnormal return", fig.height=5}
  ...
  print(ggplot(regression_cluster, 
        ...
        + geom_smooth(aes(color = company, group = company), 
                      ...
                      se = FALSE)
        + geom_point(aes(color = company, group = company))
        + labs(x = "Size of cluster per event", y = "Abnormal returns", caption = "source is own computation * Excluded anomaly"))
```
